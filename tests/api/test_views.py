...
def test_get_reco_auth(client: TestClient) -> None:
    user_id = 123
    path = GET_RECO_PATH.format(model_name="top_frequent", user_id=user_id)
    with client:
...
def test_health(
    ...
    with client:
        response = client.get("/health", headers={"Authorization": "Bearer admin"})
